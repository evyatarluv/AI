...
def h_euclidean(table, goal_table):
    """
    This h function calculate the sum of euclidean distance between current table
    to the goal table.
    :param table: current table, ndarray with shape (3,3)
    :param goal_table: gaol state table, ndarray with shape (3,3)
    :return:
    ...
    for i in range(1, 9):
        ...
        # Compute manhattan distance and sum
        x_distance = (table_pos[0][0] - goal_pos[0][0]) ** 2
        y_distance = (table_pos[1][0] - goal_pos[1][0]) ** 2
        euclidean_sum += np.sqrt(x_distance + y_distance)
